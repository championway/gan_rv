...
parser.add_argument('--dataset_name', type=str, default="pst_pix_adv_20", help='name of the dataset')
...
parser.add_argument('--gan_temp', type=int, default=20, help='interval temp of generators')
...
generator = GeneratorUNet(in_channels=3, out_channels=2)
generator_temp = GeneratorUNet(in_channels=3, out_channels=2)
discriminator = Discriminator(in_channels=4)

if cuda:
    generator = generator.cuda()
    generator_temp = generator_temp.cuda()
...
def sample_images(batches_done):
    ...
    fake_B_d = fake_B[:,1,:,:].unsqueeze(1).data
    fake_B_d = torch.cat((fake_B_d, fake_B_d, fake_B_d), 1)
    real_B_d = real_B[:,1,:,:].unsqueeze(1).data
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # GAN loss
        temp = generator_temp(real_A).detach()
        fake_B = generator(real_A)
        pred_fake = discriminator(fake_B, temp)
        ...
        # Real loss
        pred_real = discriminator(real_B, temp)
        loss_real = criterion_GAN(pred_real, valid)

        # Fake loss
        pred_fake = discriminator(fake_B.detach(), temp)
        ...
        if batches_done % opt.gan_temp == 0:
            generator_temp.load_state_dict(generator.state_dict())
